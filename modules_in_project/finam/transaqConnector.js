...
const objectAccountsAndDll = {
    ...
    dllFiles: {
        Hft: 'C:/Users/Administrator/Project/robot/modules_in_project/finam//txcn64.dll',
        NotHft: 'C:/Users/Administrator/Project/robot/modules_in_project/finam/txmlconnector64.dll',
    },
    afterInitialize: {
        Hft: {},
        NotHft: {},
...
Object.keys(typesUsersArray).forEach(number => {
    ...
    objectAccountsAndDll['afterInitialize'][typeUser] = ffi.Library(objectAccountsAndDll['dllFiles'][typeUser], dllFunctions);
});
// #endregion
...
function functionCallback(msg, HftOrNot) {
    try {
        ...
        if (inputData) {
            ...
            const dateHuman = new Date(unixTime).toISOString().replace('T', ' ').replace('Z', '');
            ...
            if (mainFile.workHereOrInTransaqConnector === false) {
                ...
                if (string.indexOf(commandText) > -1) {
                    const messageLog = `${HftOrNot } ` + `Результат выполнения команды <br>\r\n${
                        commandText.replace(/</g, '&#706;').replace(/>/g, '&#707;') }<br>\r\n` +
                        ` = <br>\r\n${
                        string.replace(/</g, '&#706;').replace(/>/g, '&#707;') }<br>\r\n`;
                    functionCloseWebServer(messageLog);
                } else
                if (
                    commandText.indexOf('gethistorydata') > -1 &&
                    string.indexOf('candles') > -1
                ) {
                    let messageLog = `${HftOrNot } ` + `Результат выполнения команды <br>\r\n${
                        commandText.replace(/</g, '&#706;').replace(/>/g, '&#707;') }<br>\r\n` +
                        ` = <br>\r\n${
                        string.replace(/</g, '&#706;').replace(/>/g, '&#707;') }<br>\r\n`;
                    ...
                    if (getHistoryByTimer === false) {
                        ...
                        console.log(`${HftOrNot } ${ messageLog.replace(/&#706;/g, '<').replace(/&#707;/g, '>').replace(/<br>/g, '')}`);
                        const object = JSON.parse(string);
                        ...
                        Object.keys(object.candles.candle).forEach(number => {
                            ...
                            const stringInsert = `INSERT INTO \`${ tableName }\` ` +
                                `(${ addId }\`birga\`, \`pair\`, \`unix_time\`, \`datetime_human\`, \`open\`, \`low\`, \`high\`, \`close\`, \`volume\`) ` +
                                'VALUES ' +
                                `(${ addIdValue } 'Finam', '${ contractString }', ${ unixTime }, ` +
                                `'${ datetimeHuman }', ${
                                object.candles.candle[number].open }, ${
                                object.candles.candle[number].low }, ${
                                object.candles.candle[number].high }, ${
                                object.candles.candle[number].close }, ${
                                object.candles.candle[number].volume })`;
            ...
            if (object.server_status !== undefined) {
                // vvv проверка обязательна
                if (object.server_status.connected === 'true') {
                    if (subscribeOnGlass[HftOrNot] === true) {
                        ...
                        if (HftOrNot === 'Hft') {
                            ...
                            // строка контракта меняется каждые 3 месяца, получить ее исходя из текущей даты
                            const contractString = functionActiveContractString();
                            const result = functionSubscribeAndUnSubscribe('subscribe', 'quotes', contractString, HftOrNot);
                        ...
                        // строка контракта меняется каждые 3 месяца, получить ее исходя из текущей даты
                        const contractString = functionActiveContractString();
                        const result = functionSubscribeAndUnSubscribe('subscribe', 'quotations', contractString, HftOrNot);
            ...
            if (object.quotations !== undefined) {
                if (
            ...
            if (object.positions !== undefined) {
                if (object.positions.forts_position !== undefined) {
                    ...
                    if (isCheckFortsPosition === true) {
                        ...
                        const diffOpenPositions = Math.abs(Number(object.positions.forts_position.todaybuy) - Number(object.positions.forts_position.todaysell));
                        if (diffOpenPositions === 0) {
                            ...
                            console.log(message);
                            writeToFile(message, `C:/Users/Administrator/Project/robot/log/${ HftOrNot }/log.log`);
                        } else {
                            isCheckFortsPosition = false;
                            const lots = Math.abs(Number(object.positions.forts_position.todaybuy) - Number(object.positions.forts_position.todaysell));
                            ...
                            let side = '';
                            let sideClose = '';
                            if (Number(object.positions.forts_position.todaybuy) > Number(object.positions.forts_position.todaysell)) {
                            ...
                            const currentUtcDateUnix = new Date(Date.UTC(Number(arrayDate['0']), (Number(arrayDate['1']) - 1), Number(arrayDate['2']))).getTime();
                            const unikString = `${currentUtcDateUnix
                                }_` +
                                'todaybuy' + `_${ Number(object.positions.forts_position.todaybuy)
                                }_` +
                                'todaysell' + `_${ Number(object.positions.forts_position.todaysell)}`;
                            ...
                            if (!arrayUnikStringCloseOpenPositions.includes(unikString)) {
                                ...
                                const message = `${HftOrNot } ${ dateHuman } ${ side } ${ lots } лотов для закрытия открытой ${ sideClose }\r\n` +
                                ...
                                const message = `${HftOrNot } ` + 'Повторная попытка отправить уже отправленное закрытие, ЗАЯВКА НА БИРЖУ НЕ ОТПРАВЛЕНА!' + `\r\n${
...
const functionCallbackHft = ffi.Callback(ref.types.bool, [ref.refType(ref.types.CString)], msg => {
...
const functionCallbackNotHft = ffi.Callback(ref.types.bool, [ref.refType(ref.types.CString)], msg => {
...
function functionConnect(HftOrNot) {
    ...
    const promise = new Promise((resolve, reject) => {
        resolve(
            // относительный путь в виндовс не всегда работает корректно, иногда существующий файл не находится
            objectAccountsAndDll['afterInitialize'][HftOrNot].Initialize(`C:/Users/Administrator/Project/robot/log/${ HftOrNot}`, 1),
        );
    });
    promise
        .then(init => {
            ...
            if (HftOrNot === 'Hft') {
                return objectAccountsAndDll['afterInitialize'][HftOrNot].SetCallback(functionCallbackHft);
            }
            if (HftOrNot === 'NotHft') {
                return objectAccountsAndDll['afterInitialize'][HftOrNot].SetCallback(functionCallbackNotHft);
            }
        ...
        .then(SetCallback => {
            ...
            const myXMLConnectString =
                `${'<command id="connect">' +
                    '<login>'}${ objectAccountsAndDll['users'][HftOrNot].Account.login }</login>` +
                ...
                '</command>';
            return objectAccountsAndDll['afterInitialize'][HftOrNot].SendCommand(myXMLConnectString);
        })
        .catch(err => {
 ...
 * @return string
 * */
function functionSubscribeAndUnSubscribe(
    operation,
    type,
    security,
    HftOrNot,
...
function functionDisconnect(HftOrNot) {
    objectAccountsAndDll['afterInitialize'][HftOrNot].SendCommand('<command id="disconnect"/>');
...
function functionConnectByTimer(HftOrNot) {
    ...
    const currentFullDateThisServer = date.toISOString().replace('T', ' ').replace('Z', '');
    ...
    const dateUnixForCancelOpenOrders = new Date(Date.UTC(Number(arrayDate['0']), (Number(arrayDate['1']) - 1), Number(arrayDate['2']), (23 + offset), 40)).getTime();
    const diffForCancel = unixTime - dateUnixForCancelOpenOrders; // -33679989
    if (
        diffForCancel >= 60000 &&
        diffForCancel <= 540000
    ) {
        ...
        Object.keys(typesOrdersArray).forEach(number => {
            ...
            if (openOrdersObject[type] !== undefined) {
                Object.keys(openOrdersObject[type]).forEach(numberInOpenOrdersObject => {
                    const transactionId = openOrdersObject[type][numberInOpenOrdersObject].transactionid;
                    if (
                        openOrdersObject[type][numberInOpenOrdersObject].status === 'watching' &&
                            openOrdersObject[type][numberInOpenOrdersObject].condition === undefined
                    ) {
                        command = 'cancelstoporder';
                        isOpenOrdersExists = true;
                    } else
                    if (
                        openOrdersObject[type][numberInOpenOrdersObject].status === 'active' ||
                    ...
                    if (command !== '') {
                        ...
                        const message = `${HftOrNot } ` + `снятие заявки transactionId = ${ transactionId }, queryObject = ${ JSON.stringify(queryObject) }, result = ${ result}`;
                        console.log(message);
                        writeToFile(message, `C:/Users/Administrator/Project/robot/log/${ HftOrNot }/log.log`);
                    }
                });
        ...
        if (
            (unixTime - startTimeTimer) >= 30000 &&
            isCheckFortsPosition === true
        ) {
            isCheckFortsPosition = false;
            const dateHuman = new Date(unixTime).toISOString().replace('T', ' ').replace('Z', '');
            const message = `${HftOrNot } ${ dateHuman } нет открытых заявок. Срабтывание в functionConnectByTimer, прошло ${ (unixTime - startTimeTimer) / 1000 } секунд.`;
            console.log(message);
            writeToFile(message, `C:/Users/Administrator/Project/robot/log/${ HftOrNot }/log.log`);
        }
    ...
    if (diffForCancel <= 900000) {
        ...
        if (
            diffForCancel >= 660000 &&
                diffForCancel < 780000
        ) {
            // 5-минутная история
...
function functionCloseWebServerByTimer() {
    if (mainFile.workHereOrInTransaqConnector === false) {
        if (getHistoryByTimer === false) {
            functionCloseWebServer('если работа веб сервера не закончена, закончить по таймеру');
        }
    }
 ...
 * @return null
 * */
function functionCloseWebServer(messageLog = '') {
    const unixTime = new Date().getTime();
    const dateHuman = new Date(unixTime).toISOString().replace('T', ' ').replace('Z', '');
    messageLog = `${dateHuman }<br>\r\n ${ messageLog}`;
    mainFile.workHereOrInTransaqConnector = true;
    // обратная замена, была для веб страницы, это для лога, разные переносы строки
    console.log(messageLog.replace(/&#706;/g, '<').replace(/&#707;/g, '>').replace(/<br>/g, ''));
    mainFile.commandText = '';
    try {
        mainFile.res.write(messageLog);
        mainFile.res.end();
    } catch (e) {
        console.log(e);
    }
...
function functionXmlQueryToSendTransactionMakeParametrsFromUrl(queryObject) {
    ...
    let stopLossPrice = parseFloat(`${ Number(orderPrice) * (1 - stopLossPercent)}`).toFixed(0);
    let takeProfitPrice = parseFloat(`${ Number(orderPrice) * (1 + takeProfitPercent)}`).toFixed(0);
    if (buyOrSell === 'S') {
        stopLossPrice = parseFloat(`${ Number(orderPrice) * (1 + takeProfitPercent)}`).toFixed(0);
        takeProfitPrice = parseFloat(`${ Number(orderPrice) * (1 - stopLossPercent)}`).toFixed(0);
    }
...
function functionCancelOrder(queryObject) {
    ...
    * */
    ...
    return objectAccountsAndDll['afterInitialize'][HftOrNot].SendCommand(makeParametrsFromUrl);
}
// #endregion
